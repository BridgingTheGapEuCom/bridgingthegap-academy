...
The API bounds login JSON bodies to 8 KiB, request headers to 16 KiB, Cookie headers to 8 KiB, and individual session-cookie values to 128 bytes; the server also limits total request reading to 10 seconds so slow login bodies cannot occupy pre-admission reads indefinitely. Authentication API responses and Problem Details use `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, and `Referrer-Policy: no-referrer`.
