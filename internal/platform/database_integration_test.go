...
func TestPostgreSQLMigrationAndSchemaCompatibility(t *testing.T) {
	...
	t.Run("plugin management read API", func(t *testing.T) { testPluginManagementHTTP(t, ctx, pool) })
	t.Run("plugin package registration HTTP API", func(t *testing.T) { testPluginPackageRegistrationHTTP(t, ctx, pool) })
	t.Run("plugin trust lifecycle HTTP API", func(t *testing.T) { testPluginLifecycleHTTP(t, ctx, pool) })
	t.Run("dashboard widget placement persistence", func(t *testing.T) { testDashboardWidgetPlacements(t, ctx, pool) })
	t.Run("dashboard widget placement HTTP API", func(t *testing.T) { testDashboardWidgetHTTP(t, ctx, pool) })
