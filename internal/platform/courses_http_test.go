...
func (r courseHTTPRepository) ListLessonsForCourseVersion(context.Context, courses.CourseVersionID) ([]courses.Lesson, error) {
...
func TestPublishedCourseHTTPReadPolicyAndCacheHeaders(t *testing.T) {
	...
	for _, check := range []struct {
		...
		{"/api/courses", http.StatusOK, "public, max-age=300"},
		{"/api/courses/event-driven-architecture", http.StatusOK, "public, max-age=60"},
		{"/api/courses/event-driven-architecture/versions/2.0.0", http.StatusOK, "public, max-age=3600"},
		{"/api/courses/event-driven-architecture/versions/2.0.0/lessons/what-is-eai", http.StatusOK, "public, max-age=3600"},
