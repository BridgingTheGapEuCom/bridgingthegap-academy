...
Each Lesson also owns one presentation-independent JSONB content document: `{ "schemaVersion": 1, "blocks": [] }`. Array order is canonical for both Continuous and Focus rendering; each block has an immutable lesson-local stable key. The built-in semantic block types are `TEXT`, `HEADING`, `IMAGE`, `VIDEO`, `AUDIO`, `CODE`, `QUOTE`, `CALLOUT`, `TABLE`, `DOWNLOAD`, `KNOWLEDGE_CHECK`, `PLUGIN_WIDGET`, and `DIVIDER`. A widget block pins an installed plugin ID, version, artifact digest, entrypoint, and bounded data-only configuration; it is rendered only through the isolated plugin runtime. The document uses a constrained rich-text model rather than HTML or editor state. Image alt/decorative rules, transcript/caption media metadata, table headers, heading levels, safe links, and bounded sizes are validated before persistence. Empty documents remain a supported migration-compatible structural state and render as an explicit empty state without Focus controls.
