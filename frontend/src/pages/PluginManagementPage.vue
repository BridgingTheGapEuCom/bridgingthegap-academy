...
async function createKey() { if (busy.value || !keyID.value || !publicKey.value || keyError.value || allowListError.value) return; const request = ++generation; busy.value = 'key'; error.value = ''; try { await addPluginKey({ keyId: keyID.value, publicKey: publicKey.value, purpose: keyPurpose.value, allowedPluginIds: keyPurpose.value === 'BTG_OWNED_SIGNING' ? allowList.value : [] }); if (!active || request !== generation) return; keyID.value = ''; publicKey.value = ''; allowedPluginIDs.value = ''; announcement.value = 'Verification key added.'; await reloadAuthoritative(); await nextTick(); heading.value?.focus() } catch (cause) { if (active && request === generation) error.value = pluginManagementMessage(cause, 'The verification key could not be added.') } finally { if (active && request === generation) busy.value = '' } }
async function setKeyEnabled(key: PluginVerificationKey, enabled: boolean) { if (busy.value) return; const request = ++generation; busy.value = 'key'; error.value = ''; try { await setPluginKeyEnabled(key.keyId, enabled); if (!active || request !== generation) return; announcement.value = enabled ? 'Verification key enabled.' : 'Verification key disabled.'; await reloadAuthoritative() } catch (cause) { if (active && request === generation) error.value = pluginManagementMessage(cause, 'The verification key could not be updated.') } finally { if (active && request === generation) busy.value = '' } }
...
.plugin-management__actions { display: flex; flex-wrap: wrap; gap: .5rem; margin-block-start: 1rem; }.plugin-management__key-form { display: grid; gap: 1rem; max-width: 48rem; }.plugin-management textarea { box-sizing: border-box; width: 100%; min-height: 7rem; font-family: ui-monospace, monospace; }.plugin-management__error, .plugin-management__warning { color: var(--color-danger, #9b1c1c); }.plugin-management__note { font-size: .95rem; }.plugin-management__version { font-size: .9em; font-weight: 400; }
