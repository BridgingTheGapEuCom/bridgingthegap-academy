<template>
  <BtgPageContainer as="section" class="community-page" aria-labelledby="community-title">
    ...
    <template v-else>
      <header><h1 id="community-title" ref="heading" tabindex="-1">Course discussions</h1><p>Discuss this course with other participants.</p></header>
...
import { communityPath, communityThreadPath, createCommunityThread, getCourseCommunity, listCommunityThreads, type CommunityThreadPage } from '../courses/community'
...
const route=useRoute(),router=useRouter(),state=ref<State>({kind:'loading'}),title=ref(''),body=ref(''),formError=ref(''),creating=ref(false),heading=ref<HTMLElement|null>(null);let active=true,generation=0
...
async function load(offset=0){const id=courseId(),request=++generation;state.value={kind:'loading'};try{const meta=await getCourseCommunity(id);const page=meta.mode==='ENABLED'?await listCommunityThreads(id,offset):{threads:[],total:0,limit:20,offset:0};if(!active||request!==generation||id!==courseId())return;state.value={kind:'ready',mode:meta.mode,page}}catch{if(active&&request===generation)state.value={kind:'unavailable'}}}
