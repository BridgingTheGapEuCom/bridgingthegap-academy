...
describe('published lesson content decoder', () => {
  ...
  it('allows only root-relative and HTTPS published links', () => {
    ...
    expect(safePublishedURL('http://example.test')).toBeUndefined()
    expect(safePublishedURL('data:text/html,<script>alert(1)</script>')).toBeUndefined()
    expect(safePublishedURL('vbscript:msgbox(1)')).toBeUndefined()
